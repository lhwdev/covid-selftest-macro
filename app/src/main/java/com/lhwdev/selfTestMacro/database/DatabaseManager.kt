...
class DatabaseManager(pref: PreferenceHolder) {
	...
	val apiLoginCache: MutableMap<UsersIdToken, UsersToken> = mutableMapOf()
	val apiUsersCache: MutableMap<String /* userCode */, User> = mutableMapOf()
	...
	suspend fun DbUser.apiUser(session: Session): User? {
		// 1. fast path #1
		apiUsersCache[userCode]?.let { return it }
		
		// 2. a-little-fast path #2
		apiLoginCache[userGroup.usersIdentifier.token]?.let { token ->
			val users = session.getUserGroup(usersInstitute, token)
			users.forEach { apiUsersCache[it.userCode] = it }
			return apiUsersCache[userCode]!!
		}
