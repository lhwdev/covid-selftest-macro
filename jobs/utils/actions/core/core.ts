import { EOL } from "https://deno.land/std@0.128.0/fs/mod.ts";
import { delimiter } from "https://deno.land/std@0.128.0/path/mod.ts";
...
export function exportVariable(name: string, val: any): void {
  ...
  Deno.env.set(name, convertedVal);

  const filePath = Deno.env.get("GITHUB_ENV") || "";
  if (filePath) {
...
export function addPath(inputPath: string): void {
  const filePath = Deno.env.get("GITHUB_PATH") || "";
  if (filePath) {
    issueFileCommand("PATH", inputPath);
  } else {
    issueCommand("add-path", {}, inputPath);
  }
  Deno.env.set("PATH", `${inputPath}${delimiter}${Deno.env.get("PATH")}`);
}
